...
export type NewUserPrefernceSchemaType = zod.infer<typeof NewUserPrefernceSchema>;

export const NewUserPrefernceSchema = zod.object({
  restricted_user_id: zod.string().min(1, { message: 'employee is required!' }),
  reason: zod.string().optional(),
});

type Props = {
  currentData?: Partial<NewUserPrefernceSchemaType>;
  sx?: BoxProps['sx'];
  onSuccess?: () => void;
...
export function UserPreferenceNewCardForm({
  sx,
  currentData,
  onSuccess,
  currentUserId,
  ...
  const methods = useForm<NewUserPrefernceSchemaType>({
    mode: 'onSubmit',
    resolver: zodResolver(NewUserPrefernceSchema),
    ...
    try {
      const endpoint =
        isEditMode && restrictionId
          ? `${endpoints.userRestrictions}/${restrictionId}`
          : endpoints.userRestrictions;
