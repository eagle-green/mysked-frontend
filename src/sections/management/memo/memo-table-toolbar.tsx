...
function CompanyWideMemoToolbarView({ filters, onResetPage, options, dateError }: Props) {
    ...
    <Box>
      ...
      >
        {/* <FormControl sx={{ width: { xs: 1, md: '100%' }, maxWidth: { xs: '100%', md: 200 } }}>
          <InputLabel htmlFor="filter-client-select">Client</InputLabel>
          <Select
            multiple
            ...
            renderValue={(selected) =>
              selected
                .map(
                  (value) => options.client.find((client) => client.value === value)?.label || value
                )
                .join(', ')
            }
            inputProps={{ id: 'filter-client-select' }}
      ...
      <Collapse in={showFilters}>
        ...
        >
          <FormControl sx={{ width: 1 }}>
            <InputLabel htmlFor="filter-client-select-mobile">Client</InputLabel>
            <Select
              multiple
              value={currentFilters.client}
              onChange={handleFilterClient}
              ...
              renderValue={(selected) =>
                selected
                  .map(
                    (value) =>
                      options.client.find((client) => client.value === value)?.label || value
                  )
                  .join(', ')
              }
              inputProps={{ id: 'filter-client-select-mobile' }}
              MenuProps={{ PaperProps: { sx: { maxHeight: 240 } } }}
            >
              {options.client.map((option) => (
                <MenuItem key={option.value} value={option.value}>
...
export const CompanyWideMemoToolbar = memo(CompanyWideMemoToolbarView);
