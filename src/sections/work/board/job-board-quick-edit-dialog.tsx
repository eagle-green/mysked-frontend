...
export function JobBoardQuickEditDialog({ open, onClose, job, onSuccess }: Props) {
  ...
  const { data: employeeVehicles } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetcher(
        `${endpoints.management.vehicle}?operator_id=${newWorker.employeeId}`
      );
  ...
  useEffect(() => {
    ...
    if (vehicles.length > 0) {
      ...
      setSelectedVehicles((prev) => {
        ...
        if (prev.size === vehicleIdsSet.size && vehicleIds.every((id: string) => prev.has(id))) {
  ...
  const { data: userListData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetcher([
        `${endpoints.management.user}/job-creation`,
        { method: 'GET' },
      ]);
  ...
  const selectedEmployeeInfo = employeeOptions.find(
    (opt: any) => opt.value === newWorker.employeeId
  );
  ...
  const filteredEmployeeOptions = (
    viewAllWorkers
      ...
      : enhancedEmployeeOptions.filter(
          (emp: any) =>
  ...
  const handlePublish = async () => {
    try {
      ...
      const normalizedWorkers = updatedWorkers.map((worker: any) => ({
        ...worker,
        start_time:
          typeof worker.start_time === 'string'
            ? worker.start_time
            : dayjs(worker.start_time).toISOString(),
        end_time:
          typeof worker.end_time === 'string'
            ? worker.end_time
      ...
      const updatedVehicles = displayVehicles.map((vehicle: any) => ({
        ...
        unit_number: vehicle.unit_number || '',
        operator:
          vehicle.operator && vehicle.operator.id
            ? {
  ...
  return (
    ...
    >
        ...
        <Stack spacing={3}>
          ...
          <Box>
            ...
            <Autocomplete
              ...
              value={displayWorkers.find((w: any) => w.id === timesheetManagerId) || null}
              onChange={(_, value) => {
              ...
              renderInput={(params) => {
                ...
                return (
                  <TextField
                    ...
                    helperText={
                      timesheetManagerError ||
                      'Required: Select a worker who will manage timesheets for this job'
                    }
                    InputProps={{
                      ...params.InputProps,
          ...
          {showAddWorkerForm && (
            ...
            >
              ...
              <Stack spacing={2}>
                ...
                {/* Time Selection - MOVED BEFORE EMPLOYEE SELECTION */}
                <Stack direction="row" spacing={2}>
                  <TimePicker
                    label="Start Time"
                  ...
                  <TimePicker
                    ...
                    onChange={(newValue) => {
                      setNewWorker({
                        ...newWorker,
                        end_time: newValue || dayjs().add(8, 'hour'),
                        employeeId: '',
                      });
                    }}
                    slotProps={{
                ...
                <Stack spacing={1}>
                  ...
                  <Autocomplete
                    fullWidth
                    size="small"
                    disabled={isConflictsFetching}
                    loading={isConflictsFetching}
                    options={filteredEmployeeOptions}
                    value={
                      filteredEmployeeOptions.find(
                        (opt: any) => opt.value === newWorker.employeeId
                      ) ||
                      enhancedEmployeeOptions.find(
                        (opt: any) => opt.value === newWorker.employeeId
                      ) ||
                      null
                    }
                    onChange={(_, value) => {
                      ...
                      const hasNonBlockingScheduleConflict =
                        value.hasScheduleConflict && !hasBlockingScheduleConflict;
                      ...
                      const hasMandatoryIssues =
                        hasBlockingScheduleConflict ||
                        hasTimeOffConflict ||
                        hasMandatoryNotPreferred ||
                        conflictResult.hasMandatoryIssues;
                      const cannotProceed = !conflictResult.canProceed;
                      ...
                      if (hasMandatoryIssues || cannotProceed) {
                        ...
                        if (viewAllWorkers) {
                          ...
                          toast.error(
                            `Cannot select ${value.label}: They have conflicting assignments or restrictions.`
                          );
                          return;
                        }
                      }
                    ...
                    getOptionDisabled={(option: any) => {
                      ...
                      const hasBlocking =
                        option.hasBlockingScheduleConflict ||
                        option.hasTimeOffConflict ||
                        option.hasMandatoryNotPreferred ||
                        hasCertificationIssues(option, newWorker.position);
                            ...
                            {conflictMessage && (
                              <Box
                                sx={{
                                  ...
                                  borderRadius: 0.5,
                                  backgroundColor:
                                    conflictColor === 'error' ? 'error.main' : 'warning.main',
                                  color:
                                    conflictColor === 'error'
                                      ? 'error.contrastText'
                  ...
                  <Box>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={
                            selectedVehicles.size === availableVehicles.length &&
                        ...
                        {availableVehicles.map((vehicle: any) => (
                          <Box
                            ...
                            sx={{
                              ...
                              borderColor: 'divider',
                              borderRadius: 1,
                              bgcolor: selectedVehicles.has(vehicle.id)
                                ? 'action.selected'
                                : 'background.neutral',
                            }}
                          >
      ...
      <ScheduleConflictDialog
        ...
        newJobStartTime={
          newWorker.start_time ? String(newWorker.start_time) : dayjs().toISOString()
        }
