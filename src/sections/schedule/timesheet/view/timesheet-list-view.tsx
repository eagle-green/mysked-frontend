...
export default function TimeSheelListView() {
  ...
  const canReset = !!currentFilters.query || currentFilters.status !== FILTER_ALL;
            ...
            {STATUS_OPTIONS.map((tab) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {
                      filterTimeCard.filter((tc: ITimeSheetTableView) =>
  ...
  if (query) {
    ...
    filtered = filtered.filter(
      (tc: ITimeSheetTableView) =>
        ...
        (tc.job.workers.length &&
          tc.job.workers.some(
            (w: IJobWorker) => findInString(q, w.first_name) || findInString(q, w.last_name)
          ))
    );
  }
  ...
  if (!dateError && startDate && endDate) {
    filtered = filtered.filter(
      (tc: ITimeSheetTableView) =>
        ...
        (dayjs(tc.startDate).isBefore(endDate, 'day') || dayjs(tc.startDate).isSame(endDate, 'day'))
    );
  }
