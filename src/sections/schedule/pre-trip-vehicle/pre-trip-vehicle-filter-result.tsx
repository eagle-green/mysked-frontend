...
export function PreTripVehicleFilterResult({ filters, totalResults, onResetPage, sx }: Props) {
  ...
  const handleRemoveVehicle = useCallback(
    (type: string) => {
      const newTypes = currentFilters.vehicles.filter((t: string) => t !== type);
      updateFilters({ type: newTypes });
      onResetPage();
    },
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={handleReset} sx={sx}>
      ...
      <FiltersBlock label="Type:" isShow={!!currentFilters.vehicles.length}>
        {currentFilters.vehicles.map((item: any) => (
          <Chip {...chipProps} key={item} label={item} onDelete={() => handleRemoveVehicle(item)} />
        ))}
      </FiltersBlock>
